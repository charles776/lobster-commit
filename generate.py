...
Full command-center design: countdown, lock ceremony, data-wall, BUY WATCH, quarantine.
"""
...
def fetch():
    ...
    return {
        ...
        'stats': stats, 'state': state, 'log': export.get('action_log', [])[:50],
...
def build(data):
    ...
    mc = datetime.datetime.now().replace(hour=15, minute=0, second=0, microsecond=0)
    next_open = mo if now_ts < mo.timestamp() else (mo + datetime.timedelta(days=1))
    secs_to_open = max(0, int(next_open.timestamp() - now_ts))
    h_to = secs_to_open // 3600
    m_to = (secs_to_open % 3600) // 60
    ...
    if datetime.datetime.now().weekday() >= 5:
        ...
        countdown_text = f'周{["一","二","三","四","五","六","日"][mon.weekday()]} {mon.strftime("%m/%d")} 开盘'

    q = data.get('quarantine', {})
    in_q = q.get('active', False)

    # ── Items with data-wall format ──
    items_html = ''
    for it in plan.get('items', []):
        ...
        if a in ('清仓','卖出'):
            row_cls = 'sell'; badge_cls = 'badge-sell'; icon = '⬤'
        elif a == '减仓':
            row_cls = 'reduce'; badge_cls = 'badge-reduce'; icon = '⬤'
        elif a == '买入':
            row_cls = 'buy'; badge_cls = 'badge-buy'; icon = '⬤'
        else:
            row_cls = 'hold'; badge_cls = 'badge-hold'; icon = '⬤'

        # Status feedback
        if status == 'done':
            row_cls += ' executed'
            status_tag = '<span class="exec-tag done-tag">✓ 09:47 已成交</span>'
        elif status == 'overridden':
            row_cls += ' overridden'
            tag = it.get('override_tag', '')
            status_tag = f'<span class="exec-tag override-tag">⚠ 覆盖: {tag}</span>'
        else:
            status_tag = '<span class="exec-tag pending-tag">待执行</span>' if a in ('清仓','卖出','减仓') else ''
        ...
        items_html += f'''<div class="item {row_cls}{crit_border}">
            ...
            <div class="item-main">
                <div class="item-head"><span class="code">{code}</span> <span class="name">{name}</span> {status_tag}</div>
                <div class="item-meta">{reason}</div>
            </div>
        </div>'''

    # ── BUY WATCH with distance ──
    bw_html = ''
    for b in bw:
        cond = b.get('trigger_cond', '')
        eta = b.get('eta', '')
        bw_html += f'''<div class="buy-item">
            <div class="bw-top"><span class="code">{b['code']}</span> {b['name']}<span class="eta">{eta}</span></div>
            <div class="trigger">{cond}</div>
        </div>'''

    # ── State badge ──
    state_label = 'PREP' if not plan_locked else 'LOCKED 🔒'
    state_cls = 'state-prep' if not plan_locked else 'state-locked'

    # ── Quarantine section ──
    q_html = ''
    if in_q:
        q_until = q.get('expires_at', '')
        q_reason = q.get('trigger_reason', '')
        q_tag = q.get('trigger_tag', '')
        q_html = f'''<div class="quarantine-banner">
            <div class="q-title">⚠ QUARANTINE ACTIVE</div>
            <div class="q-detail">触发: {q_tag} · 至 {q_until}</div>
            <div class="q-info">计划外操作已冻结。此页面只读。</div>
        </div>'''
...
<link rel="apple-touch-icon" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAMAAAADACAIAAADdvvtQAAADwElEQVR4nO3UQQpWVxCEUSeBELL/qUuN">
...
body{{font-family:'Noto Sans SC',-apple-system,sans-serif;background:#05080C;color:#E8ECF2;min-height:100vh;padding-bottom:120px}}
...
.header .state-badge{{padding:3px 10px;font-size:9px;font-family:'JetBrains Mono',monospace;font-weight:700;letter-spacing:1.5px;border-radius:2px}}
...
/* ── Items (data-wall) ── */
.item{{display:flex;gap:10px;padding:10px 8px;border-bottom:1px solid rgba(28,37,50,.35);font-size:12px;transition:all .15s;border-radius:2px;margin-bottom:1px}}
...
.item.executed{{opacity:.5}}
.item.executed .item-head{{text-decoration:line-through}}
.item.overridden{{opacity:.6;background:rgba(255,59,48,.02)}}
...
.pending-tag{{color:#FF9F0A;animation:pulse-dot 2s infinite}}
...
.lock-btn{{display:block;width:100%;padding:16px;font-size:15px;font-weight:700;cursor:pointer;border:2px solid #FF9F0A;background:rgba(5,8,12,.96);color:#FF9F0A;letter-spacing:2px;font-family:'JetBrains Mono',monospace;text-transform:uppercase;border-radius:6px;text-align:center;backdrop-filter:blur(12px);box-shadow:0 0 40px rgba(255,159,10,.08);transition:all .3s}}
...
<body>
<div class="header">
    <div><div class="date">{plan["date"]} · MONDAY</div><div class="countdown">距离开盘 {countdown_text}</div></div>
    <div class="state-badge {state_cls}">{state_label}</div>
</div>
...
<div class="install-banner" id="installBanner">
    <span>📱 安装到桌面</span>
    <button onclick="installApp()" style="background:#FF9F0A;color:#05080C;border:none;padding:6px 14px;border-radius:4px;font-weight:700;font-family:'JetBrains Mono',monospace;cursor:pointer">安装</button>
</div>
...
<div class="tab-content" id="tab1">
    {bw_html if bw else '<div style="color:#7A8290;font-size:12px;text-align:center;padding:30px;font-family:monospace">等待周末选股</div>'}
</div>
...
<div class="float-lock">
    <div class="lock-btn {'locked' if plan_locked else ''}" id="lockBtn" onclick="handleLock()">
        {'🔒 已封印' if plan_locked else '🔓 封印计划'}
        <div class="sub">{'盘中照此执行 · 覆盖将触发隔离' if plan_locked else '长按3秒封印'}</div>
    </div>
</div>
...
function installApp(){{
    if(deferredPrompt){{ deferredPrompt.prompt(); deferredPrompt.userChoice.then(function(r){{ document.getElementById('installBanner').style.display = 'none'; }}); }}
}}
...
btn.addEventListener('touchstart', function(e){{
    ...
    holdTimer = setTimeout(function(){{
        ...
        document.getElementById('toast').textContent = '🔒 计划已封印'; document.getElementById('toast').classList.add('show');
        setTimeout(function(){{ document.getElementById('toast').classList.remove('show'); }}, 2000);
        btn.classList.add('locked'); btn.innerHTML = '🔒 已封印<div class=\"sub\">盘中照此执行 · 覆盖将触发隔离</div>';
        locked = true;
    }}, 3000);
}});
btn.addEventListener('touchend', function(){{ clearTimeout(holdTimer); }});
btn.addEventListener('touchmove', function(){{ clearTimeout(holdTimer); }});
function handleLock(){{
    if (!locked){{
        if (navigator.vibrate) navigator.vibrate(30);
        document.getElementById('toast').textContent = '请长按3秒封印'; document.getElementById('toast').classList.add('show');
        setTimeout(function(){{ document.getElementById('toast').classList.remove('show'); }}, 2000);
    }}
}}
</script>
